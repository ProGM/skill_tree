...
describe RolePlay::Parser do
  ...
  describe '#setup' do
    ...
    it 'parses the acl file updating the model' do
      ...
      expect(RolePlay::Models::Role.where(name: roles).count).to eq(roles.count)
      expect(acl.roles.count).to eq(4)
  ...
  def permission(acl, role, permission)
    acl = RolePlay::Models::Acl.find_by(name: acl) if acl.is_a? String
    role = RolePlay::Models::Role.find_by(name: role) if role.is_a? String
    permission = RolePlay::Models::Permission
